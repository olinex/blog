...
假设我们有一个非常巨大的袋子, 袋子内装着人类根本数不完的玻璃球, 玻璃球只有两种颜色, 绿色和橘色**.** 现在我们需要对袋子内, 两种不同颜色的玻璃球的比例进行预测. 这是一个经典的统计学采样问题. 

我们**随机地**从袋子里面拿出N个玻璃球, 则我们可以假设, 这N个玻璃球的颜色比例, 和袋子内所有的玻璃球的颜色比例**大致相等**.  这一定是真的嘛? 很遗憾这种大致相等是有可能错误的. 但是幸运地是, 这种错误的概率是我们能够控制的.
