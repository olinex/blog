...
神经网络是如此复杂, 导致我们的训练步骤也十分复杂, 需要按照尽量优化的步骤来执行:

1. 随机初始化权重
2. 通过正向传播, 为每一个特征向量X计算其输出值h\(X\)
3. 计算损失函数, 用以表征后续不断梯度下降时, 损失函数的变化
4. 通过反向传播, 为每一个单元计算其误差和权重的损失函数偏微分
5. 通过梯度下降或其他更加高效的算法进行损失函数最小化

我们建议在训练神经网络的时候, 通过逐条代入样本来逐步训练. 

{% hint style="info" %}
神经网络的损失函数是非凸函数, 因此理论上来说梯度下降等算法并不能保证找到损失函数的全局最小值, 但幸运地是, 他们还是能够找到一个很好的局部最小值的
{% endhint %}
