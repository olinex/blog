...
在上图Margin这个范围内, 可以有无数条互相平行的直线, 这些直线都能将样本进行相同的分类, 他们可以被认为是同一种分类器, 我们从最直观的经验就能看出, 在Margin正中间的直线, 便是这一种分类器当中最优的那个. 即使我们在同一种分类器中随机挑选一个分类器, 我们也可以对无穷大的M进行一定的约束. 这种约束本质上是因为我们对目标函数集合H所包含的分类器进行了约束. 从前后的因果上来说, 只有当我们通过先验的一些假设, 对目标函数集合进行约束, 才能使得目标函数集合与样本集合之间存在约束. 因此被约束的函数集合, 在样本集合M下, 假设有一个函数g, 可以替代掉简单的函数集合大小N:
